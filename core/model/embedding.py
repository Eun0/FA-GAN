...
class RNN_ENCODER(nn.Module):
    def __init__(self, n_steps=18, ntoken=27297, ninput=300, drop_prob=0.5,
                 nhidden=256, nlayers=1, bidirectional=True, **kwargs):
        super(RNN_ENCODER, self).__init__()
        self.n_steps = n_steps # max length 
    ...
    def forward(self, captions, cap_lens, hidden, mask=None):
        # input: torch.LongTensor of size batch x n_steps(max)
...
if __name__ == "__main__":
    ...
    text = torch.randint(1, 100, (2, 18), device="cuda")

    hidden = text_encoder.init_hidden(2)
    words_embs, sent_embs = text_encoder(text, torch.tensor([18, 18]), hidden)
